		...
		for item_group, monthwise_data in salesperson_items.items():
			...
			for relevant_months in period_month_ranges:
				...
				for month in relevant_months:
					...
					value = flt(month_data.get('achieved'))
					period_data[0] += value
					totals[0] += value
...
def get_columns(filters):
	...
	columns = [_("Sales Person") + ":Link/Sales Person:180"]

	group_months = False if filters["period"] == "Monthly" else True

	for from_date, to_date in get_period_date_ranges(filters["period"], filters["fiscal_year"]):
		for label in [_("Achieved") + " (%s)"]:
			if group_months:
	...
	return columns + [_("Total Achieved") + ":Float:120"]
...
def get_achieved_details(filters, sales_person, all_sales_persons, target_item_group, item_groups):
	...
	item_details = frappe.db.sql("""
		SELECT st.sales_person, MONTHNAME(so.posting_date) as month_name,
		CASE
		...
		END as amount
		from
			`tabSales Invoice Item` soi, `tabSales Invoice` so, `tabSales Team` st
		where
			soi.parent=so.name and so.docstatus=1 and st.parent=so.name
			and so.posting_date>=%s and so.posting_date<=%s
